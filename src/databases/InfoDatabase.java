...
public abstract class InfoDatabase extends Database{
	...
	public static void updateInfo(int[] fl) {
		String query = "UPDATE "+ infoTable+" AS i SET i.happinessRating = ?, "
				+ "i.sleepInfo = ?, i.exerciseInfo = ?,"
				+ "i.meditationInfo = ?, i.socializeInfo = ? WHERE (i.id = "+currentId+""
				+ " AND i.date = ?)";
		try(PreparedStatement pstmt = connection.prepareStatement(query)){
			...
			System.out.println("Problem in updateInfo");
			e.printStackTrace();
		}
	}
